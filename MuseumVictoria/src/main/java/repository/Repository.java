...
public class Repository implements RepositoryAbstract {
	...
	private static final String SOURCE_PAGE_BASE_URL = "https://collections.museumvictoria.com.au/"; // Get id attribute
																										// from json to
																										// concatnate
	private static final String SOURCE_DATA_BASE_URL = "https://collections.museumvictoria.com.au/api/"; // Get id
																											// attribute
																											// from json
																											// to
																											// concatnate
	...
	public Result getResult(String idResult, boolean ignoreExtraProperties) throws Exception {
		...
		// Request
		String response = restTemplate.getForObject(SOURCE_DATA_BASE_URL + idResult, String.class);
		ObjectMapper mapper = new ObjectMapper();
		final ObjectNode node = mapper.readValue(response, ObjectNode.class);
		...
		if (node.has("recordType")) {
			...
			case MuseumVictoriaRecordTypeModel.ARTICLE_RECORD_TYPE:
				MuseumVictoriaArticleModel articleModel = mapper.readValue(node.toString(),
						MuseumVictoriaArticleModel.class);
			...
			case MuseumVictoriaRecordTypeModel.SPECIMEN_RECORD_TYPE:
				MuseumVictoriaSpecimenModel specimenModel = mapper.readValue(node.toString(),
						MuseumVictoriaSpecimenModel.class);
		...
		for (ObjectNode node : nodes) {
			...
			if (node.has("recordType")) {
				...
				case MuseumVictoriaRecordTypeModel.ARTICLE_RECORD_TYPE:
					...
					break;
				case MuseumVictoriaRecordTypeModel.ITEM_RECORD_TYPE:
					MuseumVictoriaItemModel itemModel = mapper.readValue(node.toString(),
							MuseumVictoriaItemModel.class);
					connectDataItem(itemModel, resultsList, ignoreExtraProperties);
					break;
				case MuseumVictoriaRecordTypeModel.SPECIMEN_RECORD_TYPE:
					MuseumVictoriaSpecimenModel specimenModel = mapper.readValue(node.toString(),
	...
	private static void connectDataArticle(MuseumVictoriaArticleModel articleModel, List<Result> results,
			boolean ignoreExtraProperties) throws IOException {
		...
		result.setID(getRepositoryID()+ "/" + articleModel.id);
	...
	private static void connectDataItem(MuseumVictoriaItemModel itemModel, List<Result> results,
			boolean ignoreExtraProperties) throws IOException {
		Result result = new Result();
	...
	private static void connectDataSpecimen(MuseumVictoriaSpecimenModel specimenModel, List<Result> results,
			boolean ignoreExtraProperties) throws IOException {
		Result result = new Result();
	...
	private static String getRepositoryName() throws IOException {
		if (repName == null) {
			...
			// input = new
			// FileInputStream("/home/rgarcia/storage/workspace/Tese-Repositorio/MuseumVictoria/src/main/java/museumvictoria.properties");
			prop.load(input);
