...
public class Repository implements RepositoryAbstract {
	...
	public List<Result> SearchByTerm(String term, boolean ignoreExtraProperties) throws Exception {
		...
		for (Object itemObject : items) {
			...
			result.getSourcePage().add(SOURCE_PAGE_BASE_URL + item.getString("id"));
			result.getSourceData().add(SOURCE_DATA_BASE_URL + item.getString("id"));
	...
	private static void connectData(String key, List<String> values, Result result, boolean ignoreExtraProperties) {
		...
		switch (key) {
			case "recordType":
				...
				break;
			case "licence":
				//This Json attribute has 3 field
				//name
				//shortname
				//
				break;
