...
public class Repository implements RepositoryAbstract {
	...
	// public static final String CSV_REPO =
	// "/home/rgarcia/storage/workspace/Tese-Repositorio/EstatuariaLisboa/src/main/java/Estatu%C3%A1ria.csv";
	...
	public Result getResult(String idResult, boolean ignoreExtraProperties) throws Exception {
		...
		CSVParser csvParser = new CSVParser(reader, CSVFormat.DEFAULT
				.withHeader("X", "Y", "OBJECTID", "COD_SIG", "IDTIPO", "INF_NOME", "INF_MORADA", "INF_TELEFONE",
						"INF_FAX", "INF_EMAIL", "INF_SITE", "INF_DESCRICAO", "INF_FONTE", "INF_MUNICIPAL", "GlobalID")
				.withIgnoreHeaderCase().withFirstRecordAsHeader().withTrim());

		for (CSVRecord record : csvParser) {
			if (idMatch(idResult, record)) {
				...
				result.setID(getRepositoryID()+"/" + record.get("OBJECTID") );
	...
	private boolean parcialMatch(String term, CSVRecord record) {
		...
		if (info_name.toLowerCase().contains(term.toLowerCase())
				|| info_descricao.toLowerCase().contains(term.toLowerCase()) || term.isEmpty())
			parcialMatch = true;

		return parcialMatch;
	}

	private boolean idMatch(String idResult, CSVRecord record) {
		String info_objectid = record.get("OBJECTID");
		return idResult.equals(info_objectid);
	}
	...
	private static String getRepositoryName() throws IOException {
		if (repName == null) {
			...
			input = new FileInputStream(
					"E:/Code/Tese-Repositorio/EstatuariaLisboa/src/main/java/estatuarialisboa.properties");
			prop.load(input);
